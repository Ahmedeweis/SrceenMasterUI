...
>
  ...
  >
    ...
    <div class="w-10 h-19 rounded-xl flex flex-col items-center justify-center">
      <span class="text-sm font-semibold">{{ getDayNumber(day.date) }}</span>
...
<div class="flex items-center gap-2 w-full">
  <img src="/img/clapperboard.png" alt="Movie" class="w-5 h-5" />
  <span class="text-gray-800 font-medium truncate max-w-[150px]" title="{{ movieTitle }}">
    {{ movieTitle }}
  </span>
  <span class="text-xs bg-purple-600 text-white px-2 py-0.5 rounded-full">PG 13</span>
</div>
            <!-- Location -->
<!-- في المكان اللي تحب تعرض فيه الاسم -->
...
<div class="flex items-center gap-2 text-gray-600 text-sm">
  ...
  <span>{{ selectedDate ? dayjs(selectedDate).format('DD-MMM-YYYY') : 'Select a date' }}</span>
</div>
            <!-- Time -->
<div class="flex items-center gap-2 text-gray-600 text-sm">
...
// ===== State =====
const allShowtimes = ref([]);        // All showtimes
const selectedDate = ref("");        // Selected date
const currentIndex = ref(0);        // Index for days
const openSections = ref(["location", "date", "suite", "time"]);
const theaters = ref([]);            // List of theaters
...
const availableShowtimes = computed(() => {
  ...
  return allShowtimes.value
    .filter(s => s.showDate === selectedDate.value)
    .map(s => s.showTime);
});
const selectedTime = ref();          // Selected time
const clickedSeats = ref([]);        // Selected seats
const daysContainer = ref(null);     // Reference to days container
// ===== Seat Layout Data =====
...
// ===== Computed =====
const days = computed(() => {
  if (!allShowtimes.value.length) return [];
  const uniqueDates = [...new Set(allShowtimes.value.map(s => s.showDate))];
...
watch(days, (newDays) => {
  if (newDays.length > 0) {
    const defaultIndex = newDays.findIndex(d => d.date === selectedDate.value);
    if (defaultIndex !== -1) {
      currentIndex.value = defaultIndex;
      scrollToDay(defaultIndex);
    } else {
      currentIndex.value = 0;
      selectedDate.value = newDays[0].date;
      scrollToDay(0);
    }
  }
});
// Watch availableShowtimes to set the first showtime as default
watch(availableShowtimes, (newShowtimes) => {
  if (newShowtimes.length > 0 && !selectedTime.value) {
    selectedTime.value = newShowtimes[0];
  }
});
// ===== API Fetch Functions =====
...
const fetchTheaters = async () => {
  try {
    ...
    const defaultTheaterId = route.query.theaterId;
    if (defaultTheaterId && theaters.value.some(t => t.id == defaultTheaterId)) {
      selectedTheaterId.value = defaultTheaterId;
    } else if (theaters.value.length > 0) {
...
const nextDay = () => {
  if (currentIndex.value < days.value.length - 1) {
    ...
    scrollToDay(currentIndex.value);
  }
};
const prevDay = () => {
